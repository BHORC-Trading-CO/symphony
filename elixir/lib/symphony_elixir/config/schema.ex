defmodule SymphonyElixir.Config.Schema do
  ...
  defmodule Tracker do
    ...
    def changeset(schema, attrs) do
      ...
      |> cast(
        attrs,
        [:kind, :endpoint, :api_key, :workspace_slug, :project_slug, :assignee, :active_states, :terminal_states],
        empty_values: []
      )
    end
  end
  ...
  defmodule Agent do
    ...
    embedded_schema do
      ...
      field(:routing, :string, default: "fixed")
    ...
    def changeset(schema, attrs) do
      ...
      |> cast(
        attrs,
        [:kind, :routing, :max_concurrent_agents, :max_turns, :max_retry_backoff_ms, :max_concurrent_agents_by_state],
        empty_values: []
      )
      |> validate_inclusion(:routing, ["fixed", "by_label"])
  ...
  defmodule ClaudeCode do
    ...
    embedded_schema do
      ...
      field(:max_turns_per_invocation, :integer, default: 50)
      field(:turn_timeout_ms, :integer, default: 3_600_000)
    end
    ...
    def changeset(schema, attrs) do
      schema
      |> cast(attrs, [:command, :permission_mode, :max_turns_per_invocation, :turn_timeout_ms], empty_values: [])
      |> validate_required([:command, :permission_mode])
      |> validate_inclusion(:permission_mode, ["default", "acceptEdits", "plan", "bypassPermissions"])
      |> validate_number(:max_turns_per_invocation, greater_than: 0)
      |> validate_number(:turn_timeout_ms, greater_than: 0)
    end
  end
  ...
  defp tracker_env_vars("plane"), do: {"PLANE_API_KEY", "PLANE_ASSIGNEE"}
  defp tracker_env_vars(_kind), do: {"LINEAR_API_KEY", "LINEAR_ASSIGNEE"}
